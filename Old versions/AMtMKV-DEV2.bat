...
if %filecount% LEQ 1 (
	...
	if exist "%debug_file%" move /y "%debug_file%" "%logdir%" >nul
	goto :EOF
)
for /f "tokens=* usebackq" %%a in ("%APPDATA%\filelist.log") do (
	set "mkvinput="%%~a""
...
for /f "tokens=* usebackq skip=%fileloop%" %%a in ("%APPDATA%\filelist.log") do (
	set "mkvinput=%mkvinput% +"%%~a""
	goto get_file_input_for_end
)
:get_file_input_for_end
...
if "%nul_output%"=="0" (
	...
	if "%errorlevel%"=="2" (
		set /a error=error+1
		echo INPUT_DIR："%vdir%">"%debug_file%"
