...
rem              G  组目录优先           -  反转顺序的前缀
rem
rem 当使用文本文档输入时，该选项不起效，将根据文本文档中的路径顺序进行排序
rem
set "dir_order=N"
rem ------------------------------------------
rem append_mode
...
for /f "tokens=* usebackq" %%a in ("%USERPROFILE%\rforbat.log") do set "opath=%%~a"
(echo [
echo   "-o",
if not "%nul_output%"=="0" (
	echo   "%opath%",
...
for /f "tokens=*" %%a in ('dir /A:-D-I-L /B /S /O:%dir_order% "%vdir%"') do (
	if /i "%%~xa"==".mp4" (
		...
		set /a non_input=non_input+1
	)
	set /a input_count=input_count+1
)
if not %input_count% GTR %non_input% (
	set "notwork=0"
	goto :EOF
)
